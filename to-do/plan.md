...
Before any per-script issues, two structural problems block the pipeline:
...
- Resource declarations (threads, memory) per rule replace the current manual thread math (within the scripts)
...
- Enables `python -m its_fun_2_map.fastp_module` style invocation (if you just wanted to run one aspect of the pipeline)
